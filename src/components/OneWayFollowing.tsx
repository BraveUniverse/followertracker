...
export default function OneWayFollowing() {
  ...
  const handleRefresh = () => {
    ...
    setSelectedProfiles([]);
    setRefreshing(true);
    fetchOneWayFollowing(0).finally(() => {
      setRefreshing(false);
    });
  };

  // Fetch one-way following (profiles that you follow but don't follow you back)
  const fetchOneWayFollowing = useCallback(async (newPage = 0) => {
    if (!isConnected || !accounts[0]) return;
    ...
    try {
      ...
      if (newPage === 0) {
        ...
        // Filter to one-way following only (those who you follow but don't follow you back)
        const oneWayFollowingAddresses = allFollowing.filter(address => 
          !followerSet.has(address.toLowerCase())
        );
        
        console.log(`Found ${oneWayFollowingAddresses.length} one-way following profiles`);
        
        // Process all profiles
        const oneWayProfiles: ProfileData[] = [];
        
        for (const address of oneWayFollowingAddresses) {
          try {
            // Get profile info
            ...
            oneWayProfiles.push({
              address,
              isMutual: false,  // Tek yönlü takipler için bu her zaman false olacak
              isSelected: false, // Başlangıçta seçili değil
              profile
            });
          } catch (error) {
        ...
        // Set hasMore based on total results
        setHasMore(oneWayProfiles.length > pageSize);
      }
      
      setPage(newPage);
    } catch (error) {
      console.error('Error fetching one-way following:', error);
  ...
  // Handle unfollow operation for selected profiles
    ...
    try {
      ...
      if (selectedProfiles.length > 1) {
        try {
          ...
          // Update state to remove unfollowed profiles
          setProfiles(current =>
          ...
          // Update state based on successful unfollows
          if (successCount > 0) {
            setProfiles(current =>
        ...
        // Update state to remove unfollowed profile
        setProfiles(current =>
  ...
  }, [isConnected, accounts, fetchOneWayFollowing, fetchUserProfile]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800">One-way Following</h1>
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-4 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-500 mt-1">
            These profiles don't follow you back
          </p>
        </div>
          ...
          {loading && profiles.length === 0 ? (
            ...
            <div className="p-6 text-center">
              <p className="text-gray-500">
                {!loading ? 'No one-way following found. Everyone you follow also follows you back!' : 'Loading profiles...'}
              </p>
            </div>
          ) : (
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">About One-way Following</h2>
        <p className="text-gray-700">
          One-way following shows profiles that you follow, but who don't follow you back.
          This can help you identify non-reciprocal connections in your network.
        </p>
        <p className="mt-2 text-gray-700">
          You can select multiple profiles and unfollow them in a single operation if desired.
